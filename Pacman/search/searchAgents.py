...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            currentPosition, cornersVisitedOrig = state
            cornersVisited = cornersVisitedOrig.copy()
...
def cornersHeuristic(state, problem):
    ...
    for i in range(len(corners)):
        if not visited[i]:
            corner = corners[i]
            manhattanDist = abs(position[0] - corner[0]) + abs(position[1] - corner[1])
            if manhattanDist > maxDistToCorner:
                maxDistToCorner = manhattanDist
    
    return maxDistToCorner
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        ...
        if dist > maxDistToFood:
            maxDistToFood = dist
    
    return maxDistToFood
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def __init__(self, gameState):
        ...
        self.initialFoodNum = gameState.getNumFood()
